...
export class ApiClient {
    ...
    static BASE_URL = "https://my.epitech.eu/api/";
     ...
     */
    async getJson(endpoint, validator, options = {}, expectedStatuses = [200]) {
     ...
     * @returns Detailed request metadata and typed response data.
     */
    async putJson(endpoint, validator, options = {}, expectedStatuses = [200]) {
        return this.requestJson(
            "PUT",
            endpoint,
            validator,
     ...
     */
    async getBlob(endpoint, options = {}, expectedStatuses = [200]) {
     ...
     * @returns Detailed request metadata and typed response data.
     */
    async requestJson(method, endpoint, validator, options, expectedStatuses) {
     ...
     */
    async requestBlob(method, endpoint, options, expectedStatuses) {
        const startedAt = performance.now();
        const url = new URL(endpoint, ApiClient.BASE_URL).toString();
        const { authenticated = true, ...requestOptions } = options;
        try {
    ...
    async buildJsonResult(
        ...
        try {
            ...
            return {
                ...
                raw: null,
                data: null,
                error: `The response body is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
                delay,
                address,
                url,
                method,
     ...
     */
    static structureFromValidation(validation) {
        if (validation.success) {
            return {
                checked: true,
                valid: true,
     ...
     * @param endpoint API endpoint relative to {@link BASE_URL}.
     */
    async createFailure(error, method, endpoint) {
        const startedAt = performance.now();
        const url = new URL(endpoint, ApiClient.BASE_URL).toString();
        return {
            success: false,
     ...
     * @param method HTTP method used for the request.
     */
    async createRequestFailure(error, startedAt, url, method) {
        return {
            success: false,
            status: 0,
            expectedStatus: false,
     ...
     */
    static async resolveAddress(url) {
        try {
